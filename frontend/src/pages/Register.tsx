...
export const Register: React.FC = () => {
  ...
  const isFirstTimeSetup = appSettings?.isFirstTimeSetup ?? false;
  ...
  // Redirect to login if signups are disabled (unless first time setup)
  useEffect(() => {
    if (!isLoading && appSettings?.signupsDisabled && !isFirstTimeSetup) {
      navigate('/login');
    }
  }, [appSettings, isLoading, isFirstTimeSetup, navigate]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 dark:bg-neutral-950 p-4">
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border-2 border-black dark:border-neutral-700 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(255,255,255,0.2)] p-8">
          ...
          {isFirstTimeSetup && (
            <div className="mb-6 p-4 bg-indigo-50 dark:bg-indigo-900/30 border-2 border-indigo-300 dark:border-indigo-700 rounded-xl text-indigo-700 dark:text-indigo-300 text-sm">
              <p className="font-bold mb-1">Welcome to ExcaliDash!</p>
              <p>This is your first time setting up. The first account created will be the administrator.</p>
            </div>
          )}
